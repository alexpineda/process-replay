...
export const CMDS = ( () => {
    const c = ( id: number, len: () => number ) => ( { id, length: () => len } );
    const fun = ( id: number, func: () => number ) => ( { id, length: func } );
    const saveLength = ( data: Buffer | BufferList ) => {
        if ( data.length < 5 ) {
            return null;
        }
    ...
    const selectLength = ( data: Buffer | BufferList ) => {
        if ( data.length < 1 ) {
            return null;
        }
        return 1 + data.readUInt8( 0 ) * 2;
    };
    const extSelectLength = ( data: Buffer | BufferList ) => {
        if ( data.length < 1 ) {
            return null;
        }
...
type StringIndexCommands = typeof CMDS;
type StringIndexCommand = StringIndexCommands[keyof StringIndexCommands];
type NumberIndexCommands = Record<number, StringIndexCommand>;
export type Commands = StringIndexCommands & NumberIndexCommands;

for ( const key of Object.keys( CMDS ) ) {
    CMDS[key].name = key;
    // eslint-disable-next-line @typescript-eslint/no-unsafe-assignment
    ( CMDS as NumberIndexCommands )[CMDS[key]!.id] = CMDS[key]!;
}
